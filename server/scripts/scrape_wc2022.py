#!/usr/bin/env python3
"""Scrape the 2022 FIFA World Cup real data from English Wikipedia into a JSON seed.

Produces {server data dir}/seed/2022.json consumed by the Rust server at boot:

    teams    — the 32 qualifiers (name, FIFA-style code, flag, strength rating, group)
...
Sources (all en.wikipedia.org):
  - "2022 FIFA World Cup Group A" .. "H"  → team codes + real fixtures
  - "2022 FIFA World Cup squads"          → per-team squad lists
...
GROUPS = "ABCDEFGH"  # 2022 had 8 groups
_POLITE = 1.2  # seconds between API calls (Wikipedia rate-limits fast scrapers)

# strength rating per team code (FIFA ranking–inspired, 2022 tournament ratings)
RATINGS = {
    "BRA": 93, "BEL": 91, "FRA": 91, "ARG": 90, "ENG": 89, "ESP": 88,
    "POR": 87, "NED": 86, "GER": 85, "CRO": 84, "URU": 83, "DEN": 82,
    "MEX": 81, "USA": 80, "SEN": 80, "SUI": 79, "POL": 78, "MAR": 77,
    "SRB": 76, "WAL": 75, "ECU": 74, "CMR": 73, "CAN": 72, "QAT": 71,
    "JPN": 70, "GHA": 69, "AUS": 68, "KOR": 67, "TUN": 66, "CRI": 65,
    "KSA": 64, "IRN": 63,
}
...
def parse_group_page(wt: str):
    ...
    box_re = re.compile(
        r"<section begin=['\"]?([A-H])(\d+)['\"]?\s*/>\s*\{\{Football box(.*?)\n\}\}",
        re.S,
    )
    ...
    while True:
        ...
        block = m.group(3)
        team1 = re.search(r"unpre\|avar=fb\|([A-Z]{3})", box_field(block, "team1"))
        team2 = re.search(r"unpe\|avar=fb\|([A-Z]{3})", box_field(block, "team2"))
        if team1 and team2:
...
def main() -> int:
    out_path = sys.argv[1] if len(sys.argv) > 1 else "data/seed/2022.json"
    ...
    for letter in GROUPS:
        page = f"2022 FIFA World Cup Group {letter}"
    ...
    print("fetching 2022 FIFA World Cup squads ...", file=sys.stderr)
    squads_by_name = parse_squads(api_wikitext("2022 FIFA World Cup squads"))
