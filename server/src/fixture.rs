...
/// Builds a per-group, per-round schedule from the stored `matches` rows
/// (real imported group fixtures). `schedule[group][round-1]` lists the
/// (home, away) pairings of that matchday. Returns `None` when the tournament
/// has no stored group fixtures, so callers fall back to the round-robin
/// schedule generated by `round_robin`.
pub fn real_group_schedule(
    conn: &Connection,
    tournament_id: i64,
) -> rusqlite::Result<Option<Vec<Vec<Vec<(i64, i64)>>>>> {
    let groups = load_groups(conn, tournament_id)?;
    if groups.is_empty() {
        return Ok(None);
    }
    let mut stmt = conn.prepare(
        "SELECT home_team_id, away_team_id, matchday FROM matches
         WHERE tournament_id = ?1 AND stage = 'GROUP' AND status = 'scheduled'
         ORDER BY matchday, id",
    )?;
    let rows: Vec<(i64, i64, Option<i32>)> = stmt
        .query_map([tournament_id], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)))?
        .collect::<Result<_, _>>()?;
    if rows.is_empty() {
        return Ok(None);
    }
    let mut schedule: Vec<Vec<Vec<(i64, i64)>>> = Vec::new();
    for (_letter, members) in &groups {
        let mut by_round: Vec<Vec<(i64, i64)>> = Vec::new();
        for (home, away, matchday) in &rows {
            if members.contains(home) || members.contains(away) {
                let round = matchday.map(|m| m.max(1) as usize).unwrap_or(1);
                while by_round.len() < round {
                    by_round.push(Vec::new());
                }
                by_round[round - 1].push((*home, *away));
            }
        }
        if by_round.iter().all(|r| r.is_empty()) {
            return Ok(None); // stored fixtures don't cover these groups
        }
